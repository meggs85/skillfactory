...
#cache_file_name = 'way-prokat_84 — копия'
...
if platform == "linux" or platform == "linux2":
    ...
    # OS X
    dbPath = os.environ['PWD'] + '/Yandex.Disk.localized/dashboard_wayprokat/db/' + cache_file_name
elif platform == "win32":
...
def SelectDealFieldsScheme():
    curs = con.cursor()
    cmd = 'select * from DealFields';
    if DEBUG: print(cmd)
    return pd.DataFrame(curs.execute(cmd).fetchall(), columns=['FIELD_NAME', 'TYPE', 'DISPLAY_TEXT', 'ISMULTIPLE', 'JSON_TEXT'])
